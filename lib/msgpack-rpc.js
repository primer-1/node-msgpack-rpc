...
var MsgpackRPCStream = function(stream, handler) {
  ...
  this.decoder = msgpack.decoder({header: false});
  this.stream.pipe(this.decoder);
  this.encoder = msgpack.encoder({header: false});
  this.encoder.pipe(this.stream);

  this.decoder.on('data', function(msg) {
    var type = msg.shift();
    switch(type) {
      case REQUEST:
        var seqid  = msg[0];
...
MsgpackRPCStream.prototype.invoke = function() {
  ...
  if(this.stream.writable) { return this.encoder.write([REQUEST, seqid, method, args]) };
}

MsgpackRPCStream.prototype.respond = function(seqid, error, result) {
  if(this.stream.writable) { return this.encoder.write([RESPONSE, seqid, error, result]) };
}
...
MsgpackRPCStream.prototype.notify = function(method, params) {
  ...
  if(this.stream.writable) { return this.encoder.write([NOTIFY, method, args]) };
}
...
exports.createClient = function(port, hostname,cb) {
  if(typeof hostname == 'function') {
    // port is actually path in this case.
    var s = new MsgpackRPCStream(new net.createConnection(port));
    s.on('ready', hostname);
    return s;
  } else {
